...
function loginToPortal(userDetails) {
  ...
  var url = "".concat(baseUrl, "api/registration/googleLogin");
  try {
...
function trySampleRequest() {
  ...
  if (params && params['access_token']) {
    ...
    xhr.open('GET', 'https://www.googleapis.com/oauth2/v2/userinfo?' + 'access_token=' + params['access_token']);
    xhr.onreadystatechange = function (e) {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          var userDetails = JSON.parse(xhr.response);
